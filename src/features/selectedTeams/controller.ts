...
export const createTeam = async (req: Request, res: Response, next: NextFunction) => {
    try {
        const temaData = selectedTeamSchema.parse(req.body);
        if (Object.keys(temaData).length === 0 || !Object.keys(temaData).every((key) => CREATE_ALLOWED.has(key))) {
            return res.status(400).json(handleResponse(400, "Bad Request"));
        }

        const isExist = await dbActions.read(SelectedTeam, {
            query: { id: temaData.id }
        });
        if (isExist) {
            return res.status(400).json(handleResponse(400, "Team with the same ID already exists"));
        }

        const team = await dbActions.create(SelectedTeam, temaData);
        if (!team) {
